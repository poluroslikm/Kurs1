...
int fgeneration(FILE* saper_file, int saper[D][D], int d1, int d2);
...
//генерация бомбы и цифр вокруг нее
int bomb(int saper[D][D], int c1, int c2)
{
	saper[c1][c2] = 9;
...
int fgeneration(FILE* saper_file, int saper[D][D], int d1, int d2)
{
	...
	for (int i1 = 1; i1 <= d1; i1++)
	{
		...
		for (int i2 = 1; i2 <= d2; ++i2)
		{
			if (saper[i1][i2] == 0) fprintf(saper_file, "  ");
			if ((saper[i1][i2] != 0) && (saper[i1][i2] < 9)) fprintf(saper_file, "%d ", saper[i1][i2]);
			if (saper[i1][i2] > 9) fprintf(saper_file, "* ");
		}
		fprintf(saper_file, "|\n");
	}
}
...
int saper(FILE* saper_file, short int random, short int base)
{
	...
	for (int l = 0; l < k; l++) //количество полей, изначально 1
	{
		...
		generation(saper, d1, d2);
		if (base == 1) 
			fgeneration(saper_file, saper, d1, d2);
	}
}